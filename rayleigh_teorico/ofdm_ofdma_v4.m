...
h1 = ((randn(1,2)+1i*randn(1,2)))/ (sqrt(2) * sqrt(2));
h2 = h1;
h3 = h1;
h4 = h1;
...
for i=1:1:length(EbNo)
        ...
        while(sum_ber_u1 < 300 && sum_ber_u2 < 600)
            i
            iteracao = iteracao + 1;
            %tx 
...
%             h4 = ((randn(1,30)+1i*randn(1,30)))/ (sqrt(2) * sqrt(30));
            ...
            Hestimado_1 = H1; %interpft((pilotos_rx_1(pilot_positions)),N);
            Hestimado_2 = H2; %interpft((pilotos_rx_2(pilot_positions)),N);
            Hestimado_3 = H3; %interpft((pilotos_rx_3(pilot_positions)),N);
            Hestimado_4 = H4; %interpft((pilotos_rx_4(pilot_positions)),N);
...
title('4 users using 16QAM - Rayleigh Teorico');
grid on;
